...
for i in range(1,41):
    try:
        ...
        colors=[]
        print(result)
        timestamp = str(int((time.time() - timest) * 10000) / 10000)
        print("The result is computed in : " , timestamp, " seconds\n")

        # Generate the time results
        with open(cwd + '/out/cp/cp_ann/time-{num}.txt'.format(num=i), 'a') as f:
            f.truncate(0)
            f.writelines('The result is computed in : '+ timestamp + ' seconds' + '\n')
            f.close()

        # Generate the output instances
        with open(cwd + '/out/cp/cp_ann/ins-{num}.txt'.format(num = i), 'a') as f:
            f.truncate(0)
